...
// const mathjax = require('mathjax/es5/tex-chtml');
...
function updateResult() {
  ...
  // 装饰mermaid绘图
  var mer = document.getElementsByClassName("language-mermaid")
  var l_me = mer.length;
  for(var i=0;i<l_me;i++){
	  var xin = document.getElementById("mermaid-svg-"+(i+1))
	  console.log('aaaaaaaaaaaaaaaaaaaaa');
	  console.log(xin);
	  var ti = document.createElement('div');
	  ti.className = 'mymermaid'
	  var jiu = xin.parentNode.parentNode;
	  xin.parentNode.parentNode.parentNode.replaceChild(ti,jiu);
	  ti.appendChild(xin)
  }
  // 数学刷新
...
//////////////////////////////////////////////////////////////////////////////
// Init on page load	页面加载时初始化
//
$(function () {
  // highlight snippet	突出显示代码段
  if (window.hljs) {
  ...
  $(document).on('click', '[data-result-as]', function (event) {
    ...
    if (view) {
      setResultView(view);
      // only to update permalink	只是为了更新perma链接
      updateResult();
      event.preventDefault();
    }
  });

  // Need to recalculate line positions on window resize	需要在调整窗口大小时重新计算行位置
